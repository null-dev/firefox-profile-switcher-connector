...
pub fn process_cmd_launch_profile(
    context: &AppContext,
    profiles: &ProfilesIniState,
    msg: NativeMessageLaunchProfile,
) -> NativeResponse {
    ...
    match notify_focus_window(context, &msg.profile_id, msg.url.clone()) {
        Ok(_) => {
            return NativeResponse::success(NativeResponseData::ProfileLaunched);
        }
        Err(e) => {
            log::info!(
                "Failed to focus current browser window, launching new window: {:?}",
                e
            );
        }
    }

...
pub fn execute_init_cmd(app_state: &mut AppState, msg: NativeMessage) -> NativeResponse {
    match msg {
        NativeMessage::Initialize(msg) => {
            process_cmd_initialize(app_state, profiles!(app_state), msg)
        }
        _ => NativeResponse::error_with_dbg_str(
            "Connector is not ready yet!",
            "Connector has not been initialized.".to_owned(),
        ),
    }
}

pub fn execute_cmd_for_message(context: &AppContext, msg: NativeMessage) -> NativeResponse {
    let state = context.state;

    match msg {
        NativeMessage::Initialize(_) => {
            NativeResponse::error("Connector cannot be initialized multiple times!")
        }
        NativeMessage::LaunchProfile(msg) => {
            process_cmd_launch_profile(context, &profiles!(state), msg)
        }
        NativeMessage::CreateProfile(msg) => {
            process_cmd_create_profile(context, profiles!(state), msg)
        }
        NativeMessage::DeleteProfile(msg) => {
            process_cmd_delete_profile(context, profiles!(state), msg)
        }
        NativeMessage::UpdateProfile(msg) => {
            process_cmd_update_profile(context, profiles!(state), msg)
        }
        NativeMessage::UpdateOptions(msg) => {
            process_cmd_update_options(context, &profiles!(state), msg)
        }
